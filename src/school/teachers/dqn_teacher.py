...
class DQNTeacher(Teacher):
    ...
    def __init__(self, nSkills: int, tasks: List[Task], timeToExam=None, noExamTasks=None,
                 **kwargs):
        ...
        self.epsilonDecreaseFactor = 1  # value between 0 and 1
        ...
        self.memFull = False
        self.epsilonDecreaseMultiplayer = (self.epsilonDecreaseFactor / self.nSkills) * 1e-5
        self.__examResults = {}
        self.__statesBuff = [0] * self.batchSize
        self.__actionsBuff = [0] * self.batchSize
        self.__realQsBuff = [None] * self.batchSize
    ...
    def receive_result(self, result, last=False, reward=None) -> None:
        ...
        # decrease randomness in teacher choices
    ...
    def __update_memory(self, result: Result, last: bool):
        ...
        if self.memIdx == self.memSize - 1:
            self.memFull = True
            # run first learning batch immediately after memory gets full
            self.learnCounter = self.noLearnIte - 1
    ...
    def __update_epsilon(self):
        if self.epsilon > self.minEpsilon:
            self.epsilon -= self.epsilonDecreaseMultiplayer * self.epsilon
    ...
    def __get_batch_tuple(self):
        ...
        for batchIdx, memIdx in enumerate(batchSelectIdxs):
            ...
            if memRecord.done:
                realQ = tf.constant([reward], dtype=tf.float32)
            else:
                nextState = tf.expand_dims(tf.constant(memRecord.nextState), axis=0)
                realQ = reward + self.gamma * self.__get_target_q(nextState)[0]
        ...
        realQs = tf.stack(self.__realQsBuff)
